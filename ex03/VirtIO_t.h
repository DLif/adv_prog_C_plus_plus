...
enum shift_set{read_e,write_e,no_set_e};

class VirtIO_t {

public:
	VirtIO_t():file_descriptor(NULL),path(NULL),mode(closed),comma_read_or_write(no_set_e),bufferPoint(NULL){};
	VirtIO_t(string filePath,size_t openMode);
	virtual ~VirtIO_t(){ fclose(file_descriptor);};

	inline VirtIO_t& operator>>(void* Buf); //this only set preperations for comma operator
	inline VirtIO_t& operator<<(const void* Buf); 
	...
	inline const string get_file_path() const;
	inline const size_t get_file_access_mode() const;

	//status getters as requested in doc.
	inline bool is_ok_e() const;
	inline bool is_cant_open_file_e() const;
	inline bool is_bad_access_e() const;
	inline bool is_writeErr_e() const;
    inline bool is_readErr_e() const;
	inline bool operator!() const; //== not is_ok_e()
	...
	inline void clear(size_t set=ok_e); //on failure throw exception and set status
...
private:
	...
	string modeTranslation(size_t ourMode);
};
